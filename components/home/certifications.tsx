            ...
            <div className="mt-8">
              {certifications.map((cert) => (
                <TabsContent key={cert.id} value={cert.id}>
                  <Card className="border-2 transition-all hover:border-primary/50 relative overflow-hidden group min-h-[350px] flex">
                    <div className="absolute inset-0 z-0 pointer-events-none bg-muted/30">
                      <Image
                        src={cert.image}
                        alt={cert.title}
                        fill
                        className="object-contain p-4 opacity-70 transition-all duration-500 group-hover:opacity-100 group-hover:scale-105 drop-shadow-lg"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-background via-background/70 to-background/20" />
                    </div>
                    <CardContent className="relative z-10 flex flex-col items-center justify-center p-8 text-center sm:p-12 w-full m-auto">
